	...
	:_size(s), _elements(new T[s]), _space(s)
{
	for (int index = 0; index < _size; ++index)
		_elements[index] = T(); //initialize with default value
}

template<class T>
inline Vector<T>::Vector(const Vector & arg) // Copy constructor
...
{
	if (this == &a) return *this;	// Self-assingment no work needed
...
{
    //std::cout << "Warning: Using operator[] without bounds checking." << std::endl;
	return _elements[i];
}

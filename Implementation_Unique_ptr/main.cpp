...
using namespace Implementation_Unique_ptr;
...
int main()
{
	...
	base2->action();
	base3->action();
    Base *basePtr = base1.get(); //get the raw pointer from unique_ptr
    basePtr->action(); //call action on the base pointer
    Base &basePtr2 = *base2; //get the raw pointer from unique_ptr
	basePtr2.action();
    getchar();
	return 0;
